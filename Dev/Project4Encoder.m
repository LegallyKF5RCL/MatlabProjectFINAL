...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%initialize
HexCharacters = dec2hex(Message);       %convert to hex
StringLen = size(HexCharacters);        %find how many characters we have
StringLen = StringLen(1,1);
CharLen = 2;                            %length of char will always be 2(hex)
TotalLen = StringLen * CharLen;         %total amount of 

%define variables and constants
Freq = 8000;              %sampling frequency (2.5x max expected freq)
BitLength = .5;           %time in seconds alloted for each bit
...
%TxMessage = int8(zeros([StringLen,CharLen])); 


%TxMessage = zeros([StringLen,CharLen]);
%rearrange things
%for i = 1:StringLen
%    for j = 1:CharLen
%            TxMessage(i, j) = int8(HexCharacters(i,j));
%    end
%end
